...
def runOnClass(args, imgs):
    ...
    numFound = 0
    for imgHash, imgs in list(d.items()):
        if len(imgs) > 1:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "inplaceDir",
        type=str,
        help="Directory of images, divided into " "subdirectories by class.",
    )
    parser.add_argument(
        "--delete",
        action="store_true",
        help="Delete the smallest duplicate images instead " "of just listing them.",
    )
    parser.add_argument(
        "--sha256", action="store_true", help="Show sha256 sum for duplicate images"
    )
