...
def stt_google_wav(audio_fname):
    ...
    req = urllib.request.Request(
        GOOGLE_SPEECH_URL, data=flac_cont, headers=hrs)
...
def get_speech_timestamp_in_seconds(silence):
    ...
    temp2 = ""
    for i in range(len(silence)-1):
        temp1 = str(convert_to_mins(silence[i])) + \
            " --> " + str(convert_to_mins(silence[i+1]))
...
def split_audio_file(time):
    ...
    if time[0] < 1:
        ...
        startTime = str(int(time[0])-1)
        duration = str(int(time[2]) - int(time[0]) + 1)
    command = "ffmpeg -ss " + startTime + " -t " + duration + \
        " -i audio.mp3 -vn -ac 1 -ar 16000 -acodec flac convert.flac"
    os.system(command)
