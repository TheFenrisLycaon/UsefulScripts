...
parser.add_argument(
    "--delete-unrecognized-extensions", action="store_true", dest="delete_unrecognized"
)
...
def artist(artistDir):
    ...
    if not args.ignore_multiple_artists:
        ...
        if len(artists) > 2:
            while True:
                print(("Warning: More than 2 artists found in '{}'.".format(artistDir)))
                print(
                    (
                        "This will move all songs to the '{}' directory.".format(
                            artistDir
                        )
                    )
                )
        ...
        for filename in filenames:
            ...
            if ext == ".mp3":
                ...
                if not args.delete_unrecognized:
                    print(
                        ("Error: Unrecognized file extension in '{}'.".format(filename))
                    )
                    sys.exit(-42)
