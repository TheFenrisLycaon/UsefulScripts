...
driver = webdriver.Chrome(executable_path=r"./src/chromedriver", options=opt)
driver.get(url)
...
for i in range(len(df.index)):
    if df.loc[df.index[i]].Name == "Amazon's Choice":
        df.loc[df.index[i], columns] = df.loc[df.index[i], columns].shift(-1, axis=0)
        df.loc[df.index[i]].Special = ["Amazon's Choice"]

for i in range(len(df.index)):
    if df.loc[df.index[i]].Price == "Limited time deal":
        df.loc[df.index[i], columns[2:-1]] = df.loc[df.index[i], columns[2:-1]].shift(
            -1, axis=0
        )
        try:
...
for i in range(len(df.index)):
    if df.loc[df.index[i]].Delivery == "FREE Delivery by Amazon":
        df.loc[df.index[i], columns[4:6]] = df.loc[df.index[i], columns[4:6]].shift(
            1, axis=0
        )

for i in range(len(df.index)):
    if df.loc[df.index[i]].Delivery == "No Cost EMI available on select car...":
        df.loc[df.index[i], columns[4:7]] = df.loc[df.index[i], columns[4:7]].shift(
            -1, axis=0
        )
...
engine = create_engine("postgresql://fenris:wtf@localhost:5432/Banao")
